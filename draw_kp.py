...
def demo(args):
    ...
    for idx, src in enumerate(source_paths):
        ...
        for dri in driving_paths:
            ...
            # imageio.mimsave(args.output, output_frames)
            os.makedirs(os.path.dirname(dri) + f'_kp_out9notanh{args.ckp}', exist_ok=True)
            imageio.imwrite(os.path.dirname(dri) + f'_kp_out9notanh{args.ckp}' + '/' + f'{idx}_'+os.path.basename(dri) , img_with_kp)
        ...
        # out_path = os.path.join(dirpath+'out', filename)
            ...
            imageio.imwrite(os.path.dirname(dri) + f'_kp_out9notanh{args.ckp}' + '/' + f'{idx}_d_'+os.path.basename(dri), img_d)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--ckp", type=int, default=85, help="Checkpoint epoch")
