...
def demo(args):
    ...
    for idx, src in enumerate(source_paths):
        ...
        for dri in driving_paths:
            ...
            # delta = delta
            feature = g_models["efe"].encode(img).max()
            print(feature)
            ...
            # imageio.mimsave(args.output, output_frames)
            os.makedirs(os.path.dirname(dri) + '_kp_out9fv150', exist_ok=True)
            imageio.imwrite(os.path.dirname(dri) + '_kp_out9fv150' + '/' + f'{idx}_'+os.path.basename(dri) , img_with_kp)
        ...
        # out_path = os.path.join(dirpath+'out', filename)
            ...
            imageio.imwrite(os.path.dirname(dri) + '_kp_out9fv150' + '/' + f'{idx}_d_'+os.path.basename(dri), img_d)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--ckp_dir", type=str, default="/media/luh/新加卷/lh/repo/code/lh_code/3.faceanimation/face-vae/ckp_1644_mainv9finalv1", help="Checkpoint dir")
    parser.add_argument("--output", type=str, default="output.gif", help="Output video")
    parser.add_argument("--ckp", type=int, default=150, help="Checkpoint epoch")
