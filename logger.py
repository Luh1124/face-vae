    ...
    def step(self):
        ...
        with tqdm(total=len(self.dataloader)) as progress_bar:
            for idx, x in enumerate(self.dataloader):
                ...
                #     train_vae = True
                train_vae = False
                losses_g, generated_d, transformed_d, neutral, kp_c, kp_s, kp_d, transformed_kp, occlusion, mask = self.g_full(s, d, s_a, d_a, train_vae)
                ...
                if idx == 300:
                    ...
                    self.visualizer.writer_vis(x, (generated_d, neutral), transformed_d, kp_c, kp_s, kp_d, transformed_kp, 
                                               occlusion, mask, 
                                               loss_dict={'index':float(idx/len(self.dataloader)),
                                                          'epoch':self.epoch,
                                                          'losess':losses})
        
        self.log_epoch(x, (generated_d, neutral), transformed_d, kp_c, kp_s, kp_d, transformed_kp, occlusion, mask)
        self.epoch += 1
...
class Visualizer:
    ...
    def visualize(self, x, generated_d, transformed_d, kp_c, kp_s, kp_d, transformed_kp, occlusion, mask):
        images = []
        s, d, s_a, d_a = x
        generated_d, neutral = generated_d
        ...
        neutral = neutral.data.cpu()
        kp_c_neutral = kp_c.data.cpu().numpy()[:, :, :2]
        neutral = np.transpose(neutral, [0, 2, 3, 1])
        images.append((neutral, kp_c_neutral))
