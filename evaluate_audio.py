...
def demo(args):
    ...
    audiovae.load_state_dict(audiovae_ckp["vae"])

    source_paths = []
    source_paths.append(args.source)
    driving_paths = []
    driving_paths.append(args.driving)
    outputpath = args.output + os.path.basename(args.driving)
    ...
    for idx, src in enumerate(source_paths):
        ...
        for dri in driving_paths:
            ...
            for frame_idx, (img, mfcc) in enumerate(zip(video_array, mfccs)):
                ...
                mfcc = torch.from_numpy(mfcc).unsqueeze(0).unsqueeze(1).cuda()
                ...
                audio_z = audiovae(mfcc, use_vae=False)[1]
                feature_d_audio = vae.decode(audio_z) / 10.

                # feature_d = vae.decode(feature_s_z/2. + feature_d_z/2.).view(-1, 16, 4, 4) / 10.

                delta_d_ori = g_models["efe"].decode(feature_d_ori, kp_c)
                delta_d_vae = g_models["efe"].decode(feature_d_vae, kp_c)
                delta_d_audio = g_models["efe"].decode(feature_d_audio, kp_c)
                ...
                imageio.imwrite(outputpath + '/' + f'{frame_idx}'.zfill(8) + '.png', img_d)
                img_d_array.append(img_d)
            
            imageio.mimsave(outputpath + '/output.gif', img_d_array, fps=25)
