...
def transform_kp(canonical_kp, yaw, pitch, roll, t, scale, delta):
    # [N,K,3] [N,] [N,] [N,] [N,3] [N,K,3]
    # y, x, z
    # w, h, d
    rot_mat = rotation_matrix_y(pitch) @ rotation_matrix_x(yaw) @ rotation_matrix_z(roll)
    transformed_kp = torch.matmul(rot_mat.unsqueeze(1), (scale*(canonical_kp + delta)).unsqueeze(-1)).squeeze(-1) + t.unsqueeze(1)
