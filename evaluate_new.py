...
def normalize_kp(kp_source, kp_driving, kp_driving_initial, adapt_movement_scale=False,
    ...
    if adapt_movement_scale:
        ...
        kp_new = kp_driving * adapt_movement_scale
    else:
...
def demo(args):
    ...
    checkpoint = torch.load(ckp_path, map_location=torch.device("cpu"))
    vae = VAE().cuda().eval()
    vae_ckp = torch.load("/home/lh/repo/code/lh/1.faceanimation/train-vae/ckp13v2/00000068-checkpoint.pth.tar", map_location=torch.device("cpu"))
    for k, v in g_models.items():
        v.cuda()
        v.load_state_dict(checkpoint[k])
        v.eval()
    ...
    for idx, src in enumerate(source_paths):
        ...
        kp_c = g_models["ckd"](s)
        # yaw, pitch, roll, t, delta = g_models["hpe_ede"](s)
        # kp_s_old, Rs = transform_kp(kp_c, yaw, pitch, roll, t, delta)
        ...
        for dri in driving_paths:
            ...
            feature_d = g_models["efe"].encode(img)
            
            feature_d_0 = feature_d * 10.
            feature_s_0 = feature_s * 10.
            feature_d = vae.generate(feature_d_0.flatten(1)) / 10.
            # feature_s = vae.generate(feature_s_0.flatten(1)) / 10.

            # feature_d_z, _ = vae.encode(feature_d_0.flatten(1))
            # feature_s_z, _ = vae.encode(feature_s_0.flatten(1))
            # feature_d = vae.decode(feature_s_z/2. + feature_d_z/2.).view(-1, 16, 4, 4) / 10.

            delta_d = g_models["efe"].decode(feature_d, kp_c_d)
            ...
            generated_d = g_models["generator"](fs, deformation, occlusion)
            generated_d = torch.cat((s, img, generated_d), dim=3)
            generated_d = generated_d.squeeze(0).data.cpu().numpy()
            generated_d = np.transpose(generated_d, [1, 2, 0])
            generated_d = generated_d.clip(0, 1)
            generated_d = (255 * generated_d).astype(np.uint8)
            # imageio.mimsave(args.output, output_frames)
            os.makedirs(os.path.dirname(dri) + 'out-newvae68', exist_ok=True)
            imageio.imwrite(os.path.dirname(dri) + 'out-newvae68' + '/' + f'{idx}_'+os.path.basename(dri) , generated_d)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--ckp_dir", type=str, default="/home/lh/repo/code/lh/1.faceanimation/face-vae/ckp_1644_main13v2_hid", help="Checkpoint dir")
    ...
    parser.add_argument("--source", type=str, default="./s", help="Source image, f for face frontalization, r for reconstruction")
    parser.add_argument("--driving", type=str, default='./d', help="Driving dir")
