...
from utils import transform_kp, transform_kp_with_new_pose, apply_imagenet_normalization
...
def demo(args):
    ...
    vae = VAE().cuda()
    vae_ckp = torch.load(args.ckp_vae, map_location=torch.device("cpu"))
    ...
    for idx, src in enumerate(source_paths):
        ...
        for dri in driving_paths:
            ...
            # feature_d_0 = feature_d/100.
            # feature_s_0 = feature_s/100.
            # feature_d = vae.generate(feature_d_0) * 100.
            # feature_s = vae.generate(feature_s_0) * 100.

            # feature_d_z, _ = vae.encode(feature_d_0)
            # feature_s_z, _ = vae.encode(feature_s_0)
            ...
            os.makedirs(os.path.dirname(dri) + 'train_vae_out-new', exist_ok=True)
            imageio.imwrite(os.path.dirname(dri) + 'train_vae_out-new' + '/' + f'{idx}_'+os.path.basename(dri) , generated_d)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--ckp_vae", type=str, default="/home/lh/repo/code/lh/1.faceanimation/train-vae/ckpfinal/00000025-checkpoint.pth.tar", help="Checkpoint dir")

    parser.add_argument("--output", type=str, default="output.gif", help="Output video")
    parser.add_argument("--ckp", type=int, default=86, help="Checkpoint epoch")
